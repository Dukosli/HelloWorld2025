...
def kNearestNeighbors(node, query, k, heap, increment=0):
    if not node:
        return
    ...
    if len(heap) < k:
        hq.heappush(heap, (-dist, increment, node))
        increment += 1
    else:
        if dist < -heap[0][0]:  # compare to farthest
            hq.heapreplace(heap, (-dist, increment, node))
            increment += 1
    ...
    kNearestNeighbors(near_branch, query, k, heap, increment)
    
    # Far branch mathematical condition to satisfy:
    # 
    if len(heap) < k or abs(diff) < -heap[0][0]:
        kNearestNeighbors(far_branch, query, k, heap, increment)
...
def rknn_id(dict_in, query, k):
    ...
    neighbor_ids = [node.id for dist, increment, node in sorted_neighbors]
